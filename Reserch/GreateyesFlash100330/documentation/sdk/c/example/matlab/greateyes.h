...
static const    int Message_NotCritical = 15;				// function stoped but there is no critical error (no valid result; catched division by zero). please try to call function again. 
...
static const	int sensorFeature_binningX = 1;
static const	int sensorFeature_cropX = 2;
...
//--------------------------------------------------------------------------------------------------------
 ...
 DLL_IMPORT_STRING int GetNumberOfSensorOutputModes(int modelID);
	// In: modelID				camera specific modelID
	//							you get this ID from the function "ConnectCamera(int& modelId, char*& modelStr, int& statusMSG, int addr)"
 ...
 DLL_IMPORT_STRING char* GetSensorOutputModeStrings(int index, int modelID);
